...
export function MarketInventions({ site }: { site: LearningSite }) {
  return (
    <section className="py-48 px-6 bg-forest text-band-foreground rounded-[4rem] mx-6 mb-48 relative overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10 text-center">
        ...
        <p className="text-xl text-band-foreground/70 font-serif max-w-2xl mx-auto mb-24 leading-relaxed italic">
            {site.marketStrategies.description}
        </p>

        <div className="grid md:grid-cols-3 gap-12 text-left">
            {site.marketStrategies.strategies.map((strategy, idx) => (
                <div key={idx} className="p-8 border border-band-border/10 rounded-3xl bg-band-foreground/5 backdrop-blur-sm hover:bg-band-foreground/10 transition-all group">
