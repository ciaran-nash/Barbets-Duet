      ...
      <AnimatePresence>
        {hoverD && !selectedD && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 mt-32 z-10 bg-band/80 backdrop-blur-md px-6 py-4 rounded-xl border border-band-border/20 text-band-foreground shadow-xl pointer-events-none text-center min-w-[200px]"
          >
            ...
            <p className="text-[10px] text-band-foreground/50 italic mt-2">Click to view details</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {selectedD && (
          <motion.div 
            ...
            className="absolute top-0 right-0 h-full w-full md:w-96 bg-band/90 backdrop-blur-xl border-l border-band-border/10 z-30 p-8 flex flex-col justify-center shadow-2xl cursor-default"
          >
            <button 
              onClick={() => setSelectedD(null)}
              className="absolute top-6 right-6 text-band-foreground/50 hover:text-band-foreground transition-colors cursor-pointer"
            >
              <X size={24} />
            </button>
            
            <div className="text-[10px] tracking-widest uppercase font-semibold text-band-foreground/60 mb-4 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-white animate-pulse"></span>
              {selectedD.subtitle}
            </div>
            <h3 className="font-serif text-3xl md:text-4xl font-light text-band-foreground mb-8 leading-tight">
              {selectedD.title}
            </h3>
            
            <div className="mb-10 text-band-foreground/70 text-sm leading-relaxed">
            ...
            <Link href={selectedD.url} onClick={() => setSelectedD(null)} className="group flex justify-between items-center bg-band-foreground text-band px-6 py-4 rounded-full font-semibold uppercase tracking-widest text-[10px] hover:bg-band-accent transition-colors w-full cursor-pointer">
              View Site Details
